...
from gen.tf_utils import policy_entropy
import copy
...
def _argmax_or_mcsearch(embedding, output_projection=None, update_embedding=True, mc_search=False):
    def loop_function(prev, _):
        ...
        if isinstance(mc_search, bool):
            ...
            prev_symbol = tf.cond(mc_search, lambda: tf.reshape(tf.multinomial(prev, 1), [-1]),
                                  lambda: tf.argmax(prev, 1))

        prev_prob = tf.nn.softmax(prev, dim=-1)
        emb_prev = embedding_ops.embedding_lookup(embedding, prev_symbol)
        # if not update_embedding:
        #     emb_prev = array_ops.stop_gradient(emb_prev)
        return emb_prev, prev_symbol, prev_prob
...
def embedding_rnn_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(scope or "embedding_rnn_seq2seq") as scope:
        ...
        # Encoder.
        encoder_cell = EmbeddingWrapper_GPU(
...
def attention_decoder(dec_in_index,
                      decoder_inputs,
                      initial_state,
                      attention_states,
                      cell,
    ...
    with variable_scope.variable_scope(
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            # for mc_position in range(1, decoder_size)
            # if foward_only= True or Mc_search = True or Feed_previous = True loop_function is not none. (for testing)
            word_index = dec_in_index[i]
            word_index = tf.cast(word_index, tf.int64)
            word_prob = prev_output
            if loop_function is not None and prev is not None:
                with variable_scope.variable_scope("loop_function", reuse=True):
                    np, word_index, word_prob = control_flow_ops.cond(tf.greater(i, mc_position), lambda: loop_function(prev, i),
                                                           lambda: f1(inp, word_index, prev))
            ...
            outputs.append(output)
            prev_output = output
        ## sample the word at last position ()
        if loop_function is not None and prev is not None:
            with variable_scope.variable_scope("loop_function", reuse=True):
...
def embedding_attention_seq2seq(encoder_inputs,
    ...
    with variable_scope.variable_scope(
        ...
        if isinstance(feed_previous, bool):
            outputs, state, sampled_words = embedding_attention_decoder(
                decoder_inputs,
                encoder_state,
                attention_states,
                cell,
                ...
                scope=scope,
                mc_position=mc_position)
            return outputs, state, encoder_state, sampled_words
        ...
        def decoder(feed_previous_bool):
            ...
            with variable_scope.variable_scope(
                ...
                if nest.is_sequence(state):
                    state_list = nest.flatten(state)
                return outputs + state_list, sampled_words

        outputs_and_state, sampled_words = control_flow_ops.cond(feed_previous,
                                                                 lambda: decoder(True),
                                                                 lambda: decoder(False))
        outputs_len = len(decoder_inputs)  # Outputs length same as decoder inputs.
        state_list = outputs_and_state[outputs_len:]
        ...
        if nest.is_sequence(encoder_state):
            state = nest.pack_sequence_as(structure=encoder_state,
                                          flat_sequence=state_list)
        return outputs_and_state[:outputs_len], state, encoder_state, sampled_words
...
def sequence_loss_by_example(logits, targets, weights, reward=None,
                             vocab_size=None,
                             average_across_timesteps=True,
                             output_projection=None,
    ...
    with ops.name_scope(name, "sequence_loss_by_example",
        ...
        ix = 0
        for logit, target, weight in zip(logits, targets, weights):
            logit_proj = nn_ops.xw_plus_b(logit, output_projection[0], output_projection[1])
            if softmax_loss_function is None:
                target = array_ops.reshape(target, [-1])
                # one_hot = tf.one_hot(target, vocab_size, on_value=1, off_value=None, axis=1)
                # one_hot =tf.cast(one_hot, tf.float32)
                # pi = tf.nn.softmax(logit_proj, dim=-1)
                # pi_stop = tf.stop_gradient(pi)
                # ent_reg = tf.log(pi_stop) + 1
                # ent_loss = tf.reduce_sum(one_hot * pi * tf.log(pi),axis=-1)
                # ent_loss = tf.multiply(one_hot, ent_loss)
                # crossent = -tf.reduce_sum(one_hot * tf.log(pi), axis=-1)
                crossent = nn_ops.sparse_softmax_cross_entropy_with_logits(
                    logits=logit_proj, labels=target)
                ent_loss_s = crossent
                ent_loss = tf.stop_gradient(ent_loss_s)
                # ent_loss = tf.reduce_sum(one_hot * pi, axis=-1)
                ent_loss = ent_loss * crossent
                # ent_loss = -tf.pow(10.0, -crossent) * crossent
            else:
                # inputs: logit   labels: target
                target = array_ops.reshape(target, [-1])
                # one_hot = tf.one_hot(target, vocab_size, on_value=1, off_value=None, axis=1)
                # one_hot =tf.cast(one_hot, tf.float32)
                # pi = tf.nn.softmax(logit_proj)
                # pi_stop = tf.stop_gradient(pi)
                # ent_reg = tf.log(pi_stop) + 1
                # ent_loss = tf.reduce_sum(one_hot * pi * tf.log(pi))
                crossent = softmax_loss_function(logit, target)
                # ent_loss = tf.reduce_sum(one_hot * pi, axis=-1)
                # ent_loss = ent_loss * (-crossent)
                ent_loss_s = crossent 
            ...
            crossent = tf.multiply(crossent, reward[ix])
            ix += 1
            log_perp_list.append(crossent * weight)
            ent_list.append(ent_loss * weight)
            # ent_list.append(policy_entropy(logit_proj) * weight)
        log_perps = math_ops.add_n(log_perp_list)
        ent = math_ops.add_n(ent_list)

        if average_across_timesteps:
            total_size = math_ops.add_n(weights)
            total_size += 1e-12  # Just to avoid division by 0 for all-0 weights.
            log_perps /= total_size
    return log_perps, ent


def sequence_loss(logits, targets, weights, reward=None,vocab_size =None,
    ...
    with ops.name_scope(name, "sequence_loss", logits + targets + weights):
        cross_ent, policy_ent = sequence_loss_by_example(
            logits, targets, weights, reward=reward,
            vocab_size = vocab_size,
            average_across_timesteps=average_across_timesteps,
            output_projection=output_projection,
            softmax_loss_function=softmax_loss_function)
        cost = math_ops.reduce_sum(cross_ent)
        policy_ent_sum = math_ops.reduce_sum(policy_ent)
        if average_across_batch:
            batch_size = array_ops.shape(targets[0])[0]
            return cost / math_ops.cast(batch_size, cost.dtype), policy_ent_sum / math_ops.cast(batch_size, cost.dtype)
        else:
            return cost, policy_ent_sum
...
def model_with_buckets(encoder_inputs, decoder_inputs, targets, weights, reward, buckets, vocab_size, batch_size,
                       seq2seq,
    ...
    with ops.name_scope(name, "model_with_buckets", all_inputs):
        for j, bucket in enumerate(buckets):
            with variable_scope.variable_scope(variable_scope.get_variable_scope(),
                ...
                bucket_outputs, decoder_states, encoder_state, sampled_words = seq2seq(encoder_inputs[:bucket[0]],
                                                                                       decoder_inputs[:bucket[1]])
                # bucket_outputs, decoder_states, encoder_state, sampled_words = seq2seq(encoder_inputs,
                #                                                                        decoder_inputs)
                outputs.append(bucket_outputs)
                # print("bucket outputs: %s" %bucket_outputs)
                encoder_states.append(encoder_state)
                sampled_words_bk.append(sampled_words)
                if per_example_loss:
                    loss, p_ent = sequence_loss_by_example(outputs[-1], targets[:bucket[1]], weights[:bucket[1]],
                                                           reward=reward[j],
                                                           vocab_size=vocab_size,
                                                           softmax_loss_function=softmax_loss_function, output_projection=output_projection)
                    losses.append(loss)
                    policy_ent.append(p_ent)
                    ...
                    loss, p_ent = sequence_loss(outputs[-1], targets[:bucket[1]], weights[:bucket[1]], reward=reward[j],vocab_size=vocab_size,
                                                softmax_loss_function=softmax_loss_function, output_projection=output_projection)
                    losses.append(loss)
                    policy_ent.append(p_ent)
                    # losses.append(sequence_loss(outputs[-1], targets[:bucket[1]], weights[:bucket[1]], softmax_loss_function=softmax_loss_function))

    return outputs, losses, encoder_states, policy_ent, sampled_words_bk
